...
class SrunPortalClient:
    """以类为中心封装认证登录逻辑"""
    def __init__(self, username, password, server_url='https://net.szu.edu.cn', verbose=False):
    ...
    def get_acid_and_ip_from_success_page(self, session, server_url):
        ...
        try:
            ...
            if not acid:
                self._log("未能从success页面获取到 acid")
            else:
                self._log(f"使用ACID: {acid}")
            if not ip:
                self._log("未能从success页面获取到 IP")
            else:
                self._log(f"使用IP: {ip}")
            return acid, ip
        except Exception as e:
            self.last_error = f"访问success页面或解析出错: {e}"
    ...
    def login(self):
        ...
        if not ac_id or not ip:
            self.last_error = "无法获取ac_id或ip，后续流程终止"
            self._log(self.last_error)
            return False

        referer = f"{self.server_url}/srun_portal_pc?ac_id={ac_id}"
        self._log(f"使用AC_ID: {ac_id}")
        ...
        self._log(f"挑战响应码: {token_response.status_code}")

        token_match = re.search(r'jQuery[\d_]+\((.*?)\)', token_response.text)
        if not token_match:
            self.last_error = "无法解析token响应"
            self._log(self.last_error)
            return False

        token_data = json.loads(token_match.group(1))
        if token_data['error'] != 'ok':
            self.last_error = f"获取token失败: {token_data.get('error_msg', '')}"
            self._log(self.last_error)
            return False
        ...
        self._log(f"登录响应码: {login_response.status_code}")

        try:
            login_match = re.search(r'jQuery[\d_]+\((.*?)\)', login_response.text)
            if not login_match:
                self.last_error = "无法解析登录响应"
                self._log(self.last_error)
                return False
            login_data = json.loads(login_match.group(1))
            if login_data['error'] == 'ok':
                self.last_error = login_data.get('suc_msg', 'ok')
                self._log("登录成功")
                return True
            else:
                self.last_error = f"登录失败: {login_data.get('error_msg', login_data['error'])}"
                self._log(self.last_error)
